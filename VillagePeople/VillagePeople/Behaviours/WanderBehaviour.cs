...
namespace VillagePeople.Behaviours {
    internal class WanderBehaviour : SteeringBehaviour {
        ...
        public override Vector2D Calculate() {
            _wanderTarget = _wanderTarget + new Vector2D(_random.Next(-WanderJitter, WanderJitter),
                                _random.Next(-WanderJitter, WanderJitter));
            _wanderTarget = _wanderTarget.Normalize();
            _wanderTarget *= WanderRadius / 2;

            _circleCenterG = new PointF(_self.Heading.X * WanderDistance + _self.Position.X - WanderRadius / 2,
                _self.Heading.Y * WanderDistance + _self.Position.Y - WanderRadius / 2);
            var circleCenterM = new Vector2D(_self.Heading.X * WanderDistance + _self.Position.X,
                _self.Heading.Y * WanderDistance + _self.Position.Y);
        ...
        public override void RenderSB(Graphics g) {
            if (_pointOnCircle == null) return;
            g.DrawEllipse(Pens.Yellow,
                new RectangleF(new PointF(_circleCenterG.X, _circleCenterG.Y), new SizeF(WanderRadius, WanderRadius)));
            g.FillEllipse(Brushes.Red,
                new RectangleF(new PointF(_pointOnCircle.X - 4, _pointOnCircle.Y - 4), new SizeF(8, 8)));
        }
    }
}