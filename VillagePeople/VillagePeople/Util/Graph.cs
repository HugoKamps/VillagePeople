...
namespace VillagePeople.Util {
    public class Graph {
        ...
        public Node GetClosestNode(Vector2D worldPos) {
            return Nodes.Select(x => x).OrderBy(x => GetDistance(x.WorldPosition, worldPos)).First();
        }
        ...
        private float GetDistance(float oX, float oY, float tX, float tY) {
            return GetDistance(Math.Abs(oX - tX), Math.Abs(oY - tY));
        }

        private float GetDistance(Vector2D v1, Vector2D v2) {
